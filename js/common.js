 ...
 * @param parentId
 */
function quickBuy(goodsId, parentId)
{ var goods = new Object();
    var spec_arr = new Array();
    var fittings_arr = new Array();
    var number = 1;
...
// 检查是否有商品规格
    ...
    goods.number = number;
    goods.parent = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
    Ajax.call('flow.php?step=quick_buy', 'goods=' + goods.toJSONString(), quickBuyResponse, 'POST', 'JSON');
}
 ...
 * 获得选定的商品属性 define
 */
function getSelectedAttributesBySelect(formBuy)
{
  var spec_arr = new Array();
  var j = 0;
  ...
  for (i = 0; i < formBuy.elements.length; i ++ )
  {
    ...
    if (prefix == 'spec_' && formBuy.elements[i].type=='button'&&formBuy.elements[i].className=='active')
    {
      spec_arr[j] = formBuy.elements[i].id;
      j++ ;
    }
  }
 ...
 * 直接购买处理添加商品到购物车的反馈信息
 */
function quickBuyResponse(result)
